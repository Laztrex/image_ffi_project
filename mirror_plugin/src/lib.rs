...
/// Возвращает версию API плагина.
#[no_mangle]
pub extern "C" fn plugin_api_version() -> u32 {
    1
}

/// Отражает изображение по горизонтали и/или вертикали.
///
/// # Safety
...
///
/// # Behavior
/// - `horizontal == true`, изображение отражается зеркально по горизонтали (слева направо).
/// - `vertical == true`, изображение отражается по вертикали (сверху вниз) – строки пикселей меняются местами.
/// - Оба эффекта можно применить одновременно.
///
/// # Returns
/// - `0` – успешное выполнение.
/// - `1` – передан нулевой указатель `rgba_data`.
/// - `2` – переполнение при вычислении размера буфера (слишком большое изображение).
/// - Ошибка парсинга JSON или невалидная UTF-8 строка логируются, но не прерывают выполнение (используются значения по умолчанию).
#[no_mangle]
pub unsafe extern "C" fn process_image(
    ...
    // Горизонтальное отражение: обмен пикселей внутри каждой строки
    ...
    // Вертикальное отражение: обмен строк целиком через split_at_mut
    if mirror_params.vertical {
        let row_size = w * 4;
        for row in 0..h / 2 {
            let top_start = row * row_size;
            let bottom_start = (h - 1 - row) * row_size;
            // Используем split_at_mut для получения двух непересекающихся mutable-срезов
