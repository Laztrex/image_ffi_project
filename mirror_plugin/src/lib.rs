...
/// Отражает изображение согласно переданным параметрам (без лишних копий).
/// # Safety
...
pub unsafe extern "C" fn process_image(
    ...
    // Безопасное вычисление длины с проверкой переполнения
    ...
    if mirror_params.horizontal {
        for row in 0..h {
            ...
            for col in 0..w / 2 {
                let left = row_start + col * 4;
                let right = row_start + (w - 1 - col) * 4;
                for i in 0..4 {
    ...
    if mirror_params.vertical {
        for row in 0..h / 2 {
            let top_row_start = row * w * 4;
            let bottom_row_start = (h - 1 - row) * w * 4;
            for col in 0..w {
                let top = top_row_start + col * 4;
                let bottom = bottom_row_start + col * 4;
                for i in 0..4 {
